...
const artistasSolistas = (array) => {
  return array.filter((artista) => artista.solista === true);
};
console.log(artistasSolistas(artistas));

// artistasPorEdad, que tome un parámetro "edad" y un array de artistas
//y devuelva un array con les artistas que tengan dicha edad
const artistasPorEdad = (array, edad) => {
  const artistasFiltrados = array.filter((artista) => {
    return artista.edad === edad;
  });

  return artistasFiltrados;
};
console.log(artistasPorEdad(artistas, 30));

// artistasConMasDiscosQue, que tome por parámetro "cantidadDeDiscos" y un array de artistas
...
//artistaConMasEntradasVendidas, que tome por parámetro un array de artistas y devuelva
//el objeto artista que vendió más entradas en su último recital

const artistaConMasEntradasVendidas = (array) => {
  const artistaConMasEntVend = array.reduce((acc, cur) => {
    if (
      acc.ultimoRecital.entradasVendidas > cur.ultimoRecital.entradasVendidas
    ) {
      return acc;
    } else {
      return cur;
